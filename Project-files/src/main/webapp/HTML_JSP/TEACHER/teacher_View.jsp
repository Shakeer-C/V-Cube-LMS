...
<body>
    ...
    <form action="TEACHER/Teacher_Logout" method="post" name="admin_logout">
        <button type="submit" class="btn" onclick="return confirm('Are you sure?')">Logout</button>
				...
				onclick="profileEdit('<%=ct.getStId()%>', '<%=ct.getStName()%>', '<%=ct.getStEmail()%>', '<%=ct.getStMobile()%>', '<%=ct.getStPsw()%>','<%=ct.getCrId()%>')">Profile edit</button>
		...
		<div class="rightSide-container">
			...
			<div class="rightBottom">
				...
				<h1>All Student Count : <%=request.getAttribute("studentCount") %></h1>
					<h2 style="text-align: center;">Teacher Dashboard Overview</h2>
				...
				<div>
					...
					<table border="1"
						...
						<tr>
							<td><%=c.getConTitle()%></td>
							<td><%=c.getConDisp()%></td>
							<td><a href="<%=c.getConMetiral()%>" style="color:black;">Link</a></td>
							<td><%=c.getConAssig()%></td>
							<td><%=c.getBname()%></td>
							<td><%=c.getStId()%></td>
							<td><%=c.getCrId()%></td>
							<td><form action="TEACHER/ST_ConDelete" method="post"
				...
				<h1>Content Upload</h1>
					<form action="TEACHER/ST_ConUpload" method="post" name="st_conupload"
						class="st-RForm">
				...
				<div class="set-Form" id="section4">
					<form action="AdminST_Update" method="post" name="adminst_update"
						...
						<input type="hidden" id="uCrId" name="crId"
							value="<%=session.getAttribute("crId")%>">
							<input type="hidden" id="uStId" name="stId" required>
				...
				<h2 style="text-align: center;">Profile Update</h2>
					<form action="TEACHER/ST_TprofileUpdate" method="post" name="st_tprofileupdate"
						class="st-RForm">
